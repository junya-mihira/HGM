...
int main(void) {
    ...
    while (fgets(line, sizeof(line), file) && rows < MAX_ROWS) {
        x[rows] = malloc(2 * sizeof(double)); // 2列確保(S^1球面).
        char *ptr = line;
        if (!x[rows]) {
    ...
    for (double B=0.1; B<10.0; B+=0.1){
        for (int C=1; C<=1000;C++){
            ...
            // ルンゲクッタ4次 (RK4) に変更
            gsl_odeiv2_driver *d = gsl_odeiv2_driver_alloc_y_new(&sys, gsl_odeiv2_step_rk4, 1e-2, 1e-6, 1e-6);
